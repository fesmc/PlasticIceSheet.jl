...
# z_s whose driving slope τ/(ρ_i g H) blows up as H→0). The margin Dirichlet value is the
...
#   :hj   → F = √(G² − 4√w(∇z_b·∇w) − 4w|∇z_b|²)   (depends on w through the gradient term)
# Both the forward sweep and the implicit-diff residual call this, so they cannot
# drift apart.
function _driving_rhs(w, τf, z_b, dx, dy, p::PlasticParams, mode::Symbol)
    G = @. 2 * τf / (p.ρ_i * p.g)
    mode === :flat && return G
...
function _solve_w(τf, z_b, mask, bc, dx, dy, p::PlasticParams, mode::Symbol;
                  max_sweeps::Int, tol, n_outer::Int, outer_tol, relax = 0.5)
    G = _driving_rhs(τf, τf, z_b, dx, dy, p, :flat)   # flat RHS (w arg unused for :flat)
    w = sweep_eikonal(G, mask, bc, dx, dy; max_sweeps, tol)
    if mode === :hj
        for _ in 1:n_outer
            F = _driving_rhs(w, τf, z_b, dx, dy, p, :hj)
...
"""
    godunov_residual(w, τf, z_b, mask, bc, dx, dy, params, mode)
...
"""
function godunov_residual(w, τf, z_b, mask, bc, dx, dy, p::PlasticParams, mode::Symbol)
    F = _driving_rhs(w, τf, z_b, dx, dy, p, mode)
    nx, ny = size(w)
...
"""
    solve(z_b, τ, mask; dx, dy, mode=:hj, params=PlasticParams(),
...
"""
function solve(z_b::AbstractMatrix, τ, mask::AbstractMatrix{Bool};
    ...
    p = params
    τf = τ isa Number ? fill(float(τ), size(z_b)) : τ
    size(τf) == size(z_b) || throw(DimensionMismatch("τ and z_b must match"))

    bc = _bc_flat.(z_b, Ref(p))
    w = _solve_w(τf, z_b, mask, bc, dx, dy, p, mode; max_sweeps, tol, n_outer, outer_tol, relax)
